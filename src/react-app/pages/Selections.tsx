...
export default function Selections() {
  ...
  const handlePauseAllAds = async () => {
    if (!selectedSelection || !confirm('Tem certeza que deseja pausar TODOS os anúncios desta seleção?')) return;
      ...
      <div>
        ...
        <main className="max-w-7xl mx-auto px-4 lg:px-8 py-8">
          ...
          <div className="flex justify-between items-center mb-6">
            ...
            {selectedSelection.selection_type === 'pause' && adDetails.some(ad => ad.effective_status === 'ACTIVE') && (
              <button
                onClick={handlePauseAllAds}
                disabled={pausingAds.size > 0}
              ...
              >
                ...
                <span>Pausar Todos os Ativos</span>
              </button>
            )}
          </div>
                ...
                {adDetails.map((ad) => (
                  <div key={ad.ad_id} className="p-6 hover:bg-slate-50 transition-colors">
                    <div className="flex items-center justify-between">
                      ...
                      {selectedSelection.selection_type === 'pause' && ad.effective_status === 'ACTIVE' && (
                        <button
                          onClick={() => handlePauseAd(ad.ad_id)}
                          disabled={pausingAds.has(ad.ad_id)}
                          className="ml-4 bg-yellow-600 text-white px-3 py-1 rounded-lg hover:bg-yellow-700 transition-colors disabled:bg-yellow-400 flex items-center space-x-1"
                        >
