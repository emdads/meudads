...
export default function AdsActive() {
  ...
  const handleUpdateAll = async () => {
    ...
    console.log(`[SYNC-${requestId}] Iniciando sincronização para: ${selectedAccount?.account_name}`);
    
    // Validação básica
    if (!selectedAccount?.id || !slug) {
      console.warn(`[SYNC-${requestId}] Dados obrigatórios ausentes`);
      return;
    }
    ...
    setError(null);
    setSyncProgress(5);
    setSyncStep('🔄 Preparando sincronização...');
    ...
    const globalTimeout = setTimeout(() => {
      console.log(`[SYNC-${requestId}] ⏰ Timeout forçado após 2 minutos`);
      clearInterval(progressTimer);
      setSyncStep('⏱️ Operação expirou - Sistema ocupado\n\nTente novamente em alguns minutos');
    ...
    try {
      // ETAPA 1: Conectar
      setSyncProgress(15);
      setSyncStep('🔗 Conectando com a plataforma...');
      
      const controller = new AbortController();
      const requestTimeout = setTimeout(() => {
        controller.abort();
      }, 90000); // 1.5 minutos para a requisição
      ...
      try {
        ...
        if (fetchError.name === 'AbortError') {
          throw new Error('Conexão muito lenta - Sistema ocupado');
        }
      ...
      if (!response.ok) {
        ...
        try {
          ...
          // Se não conseguiu fazer parse do JSON, tentar como texto
          try {
        ...
        // PRESERVAR ERRO ESPECÍFICO E DETALHADO DA META (com múltiplas fontes)
        ...
        // USAR DIRETAMENTE O ERRO PRINCIPAL SEM MODIFICAÇÕES
        let finalError = metaError;
        
        // Se há detalhes adicionais importantes, incluir
        if (originalError && originalError !== metaError && originalError.length > 0) {
          finalError += `\n\nDetalhes técnicos: ${originalError}`;
        }
        if (metaApiDetails && metaApiDetails !== metaError && metaApiDetails.length > 0) {
          finalError += `\n\nMeta API: ${metaApiDetails}`;
        }
        
        // Para rate limit, ser ainda mais específico
        if (errorCategory === 'rate_limit' || metaError.includes('rate limit') || metaError.includes('Rate limit')) {
          console.log(`[SYNC-${requestId}] Rate limit detectado - preservando erro específico`);
          // Manter o erro exato como está
        }
        
        console.log(`[SYNC-${requestId}] Erro final que será lançado:`, finalError);
      ...
      if (!data.ok) {
        ...
        // PRESERVAR ERRO ESPECÍFICO E DETALHADO DA META (com múltiplas fontes)
        const metaError = data.error || data.error_details || data.message || 'Falha na sincronização';
        ...
        // Se há erro original mais específico, usar ele
        ...
        // Se há detalhes específicos da Meta API, usar como complemento
        if (metaApiDetails && metaApiDetails.length > 0 && metaApiDetails !== finalError) {
          finalError += `\n\nMeta API: ${metaApiDetails}`;
        }
        
        console.log(`[SYNC-${requestId}] Erro final extraído de data.ok=false:`, finalError);
        throw new Error(finalError);
      }
      
      // ETAPA 2: Processar resultados
      setSyncProgress(70);
      setSyncStep('📊 Processando dados...');
      
      const summary = data.summary || {};
      
      // ETAPA 3: Atualizar interface
      setSyncProgress(90);
      setSyncStep('🔄 Atualizando interface...');
      ...
      let resultMessage = '✅ Sincronização Concluída!\n\n';
      
      // Mostrar estatísticas de anúncios
      const results = [];
      if (summary.ads_inserted > 0) results.push(`📈 ${summary.ads_inserted} novos anúncios`);
      if (summary.ads_updated > 0) results.push(`🔄 ${summary.ads_updated} anúncios atualizados`);
      if (summary.ads_deleted > 0) results.push(`📉 ${summary.ads_deleted} anúncios removidos`);
      if (summary.campaigns_updated > 0) results.push(`🎯 ${summary.campaigns_updated} campanhas`);
      
      if (results.length > 0) {
        resultMessage += results.join('\n') + '\n\n';
      }
      
      // Mostrar DETALHES ESPECÍFICOS das métricas por período
      ...
      if (totalMetrics > 0) {
        resultMessage += `📊 Métricas Sincronizadas:\n`;
        if (metrics7d > 0) resultMessage += `   • 7 dias: ${metrics7d} anúncios\n`;
        if (metrics14d > 0) resultMessage += `   • 14 dias: ${metrics14d} anúncios\n`;
        if (metrics30d > 0) resultMessage += `   • 30 dias: ${metrics30d} anúncios\n`;
        resultMessage += `   Total: ${totalMetrics} registros de métricas\n\n`;
      }
      
      if (summary.total_current_ads) {
        resultMessage += `🎯 Total atual: ${summary.total_current_ads} anúncios ativos\n`;
      }
      
      const duration = summary.duration_ms ? `${(summary.duration_ms / 1000).toFixed(1)}s` : 'N/A';
      resultMessage += `⏱️ Tempo: ${duration}`;
      
      // Adicionar informações sobre regras aplicadas
      if (summary.ads_deleted > 0) {
        resultMessage += `\n\n📋 Regras aplicadas:\n• Anúncios inativos foram removidos do sistema\n• Métricas foram atualizadas para anúncios ativos`;
      }
      ...
      console.log(`[SYNC-${requestId}] ✅ Concluído: ${summary.total_current_ads || 0} anúncios, ${totalMetrics} métricas`);
      ...
      if (syncError.message?.includes('An active access token')) {
        ...
        // Mostrar erro completo da Meta para diagnóstico
        const details = syncError.message || 'Erro técnico';
        errorMessage += `💥 Erro detalhado:\n${details}`;
        
        // Adicionar dicas baseadas no tipo de erro
        if (details.includes('permission')) {
          errorMessage += '\n\n💡 Dica: Verifique as permissões da conta no Meta Business Manager';
        } else if (details.includes('network')) {
          errorMessage += '\n\n💡 Dica: Problema de rede - tente novamente';
        } else if (details.includes('timeout')) {
          errorMessage += '\n\n💡 Dica: Meta muito lento - tente com menos contas por vez';
        }
      }
  ...
  const getLastSyncStatus = () => {
    ...
    try {
      ...
      if (diffMinutes < 15) {
        ...
        return {
          canSync: true,
          message: `Sincronização há mais de 24h. Recomendado atualizar.`,
          isAutoSynced: false
        };
      }
    ...
    try {
      ...
      for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
        ...
        try {
          ...
          // Se hoje é 05/09, dados vão até 04/09
          const endDate = new Date(now);
          endDate.setDate(now.getDate() - 1);
          
          // Data inicial: X dias antes da data final 
          // Para 7 dias: se final=04/09, início=04/09-(7-1)=29/08
          ...
          console.log(`[METRICS-DATES-FIX] Enviando para API - Período: ${startDateStr} até ${endDateStr} (${metricsDays} dias exatos)`);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50">
      ...
      <main className="max-w-7xl mx-auto px-4 lg:px-8 py-8">
        ...
        {/* SISTEMA DE SINCRONIZAÇÃO CONSOLIDADO */}
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h3 className="font-semibold text-slate-900">
                      {updatingAll ? 'Sincronização em Andamento' : 'Carregando Dados'}
                    </h3>
                  </div>
                </div>
              ...
              {!syncStep && (
                <div className="bg-slate-50 border border-slate-200 rounded-lg p-4">
                  <div className="text-sm text-slate-600">
                    {updatingAll 
                      ? 'Preparando sincronização com a plataforma...'
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-2 text-sm">
                        ...
                        <span className="text-blue-800 font-semibold">
                          {(() => {
                            // LÓGICA UNIFICADA: Baseada na data/hora atual do sistema
                            const now = new Date();
                            
                            // Data final: ONTEM (último dia com dados completos)
                            // Se agora é 05/09 23h48, dados vão até 04/09
                            const endDate = new Date(now);
                            endDate.setDate(now.getDate() - 1);
                            
                            // Data inicial: X dias antes da data final  
                            // Para 7 dias: se final=04/09, início=29/08
                            const startDate = new Date(endDate);
                            startDate.setDate(endDate.getDate() - (metricsDays - 1));
