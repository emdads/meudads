...
export default function AdAccountSelector({ 
    ...
    try {
      ...
      if (showAllAccounts) {
        ...
        if (response.ok) {
          ...
          setError(`Erro ao carregar contas: ${errorData.error || 'Erro desconhecido'}`);
        }
      } else {
  ...
  // Show syncing state ONLY if explicitly syncing
  if (isSyncing && selectedAccount && syncingMessage) {
    return (
      <div className="flex items-center space-x-2 bg-blue-50 rounded-lg border border-blue-200 px-3 py-2">
        <div className="animate-spin w-4 h-4 border-2 border-blue-600 border-t-transparent rounded-full"></div>
        <div className="flex-1">
          <div className="text-sm font-medium text-blue-800">
            Sincronizando: {selectedAccount.account_name}
          </div>
          <div className="text-xs text-blue-600 mt-1">
            {syncingMessage}
          </div>
