...
// Run scheduled syncs periodically (5% chance per request)
app.use("*", async (c, next) => {
  if (Math.random() < 0.05) {
    // Run in background without blocking the request
    runScheduledSyncs(c.env.DB, c.env).catch(error => {
      console.error('[SCHEDULED-SYNC] Background sync error:', error);
    });
  }
  await next();
});
...
app.get("/api/admin/clients/:slug/ad-accounts/:accountId/sync", 
  ...
  async (c) => {
    ...
    try {
      ...
      // STEP 8: ULTRA-SAFE PRE-SYNC SETUP
      responseData.stage = 'pre_sync_setup';
      console.log(`[SYNC-ULTRA-SAFE-${requestId}] Setting up sync...`);
      ...
      try {
        ...
        `).bind(accountId).run();
        console.log(`[SYNC-ULTRA-SAFE-${requestId}] ✅ Account status updated to 'syncing'`);
      ...
      try {
        ...
        try {
          const errorMessage = syncError instanceof Error ? syncError.message.substring(0, 500) : 'Platform sync error';
          await c.env.DB.prepare(`
      ...
      // STEP 10: ULTRA-SAFE METRICS SYNC (ONLY IF PLATFORM SYNC SUCCEEDED)
      let metricsResults = { days_7: 0, days_14: 0, days_30: 0, metrics_errors: 0 };
      
      if (syncResult?.ok && isPlatformSupported(adAccount.platform, 'metricsSupported')) {
        console.log(`[SYNC-ULTRA-SAFE-${requestId}] 📊 Starting ultra-safe metrics sync for periods 7, 14, 30 days...`);
        
        try {
          const metricsCache = new MetricsCache(c.env.DB);
          
          // Ultra-safe metrics sync for each period
          for (const metricsDays of [7, 14, 30]) {
            try {
              console.log(`[SYNC-ULTRA-SAFE-${requestId}] Syncing metrics for ${metricsDays} days...`);
              
              // Add timeout for each metrics period
              const metricsTimeoutMs = 60000; // 1 minute per period
              const metricsPromise = metricsCache.syncMetricsForPeriod(
                accountId, 
                client.id, 
                metricsDays
              );
              ...
              if (metricsResult && typeof metricsResult === 'object') {
                ...
                console.log(`[SYNC-ULTRA-SAFE-${requestId}] ✅ Metrics for ${metricsDays} days: ${(metricsResult as any)?.success || 0} success, ${(metricsResult as any)?.errors || 0} errors`);
              } else {
                console.warn(`[SYNC-ULTRA-SAFE-${requestId}] ⚠️ Invalid metrics result for ${metricsDays} days`);
                metricsResults.metrics_errors++;
              }
              
              // Safe delay between periods to avoid rate limits
              if (metricsDays !== 30) {
                await new Promise(resolve => setTimeout(resolve, 1500));
              }
              
            } catch (periodError: any) {
              console.error(`[SYNC-ULTRA-SAFE-${requestId}] ❌ Error syncing metrics for ${metricsDays} days:`, periodError);
              metricsResults.metrics_errors++;
              
              // Don't fail entire sync for metrics errors
              if (periodError.message?.includes(`METRICS_TIMEOUT_${metricsDays}d`)) {
                console.warn(`[SYNC-ULTRA-SAFE-${requestId}] ⚠️ Metrics timeout for ${metricsDays} days (non-critical)`);
              }
            }
          }
          
          const totalMetrics = metricsResults.days_7 + metricsResults.days_14 + metricsResults.days_30;
          console.log(`[SYNC-ULTRA-SAFE-${requestId}] ✅ Total metrics synced: ${totalMetrics}, errors: ${metricsResults.metrics_errors}`);
          
        } catch (generalMetricsError) {
          console.error(`[SYNC-ULTRA-SAFE-${requestId}] ❌ General metrics sync error (non-critical):`, generalMetricsError);
          metricsResults.metrics_errors++;
          // Don't fail sync for metrics errors
        }
      } else {
        console.log(`[SYNC-ULTRA-SAFE-${requestId}] ⚠️ Metrics not supported for platform: ${adAccount.platform} or sync failed`);
      }
      ...
      if (syncResult?.ok) {
        ...
        try {
          const errorToStore = (syncResult?.error || 'Erro desconhecido na sincronização').substring(0, 500);
          await c.env.DB.prepare(`
            UPDATE ad_accounts SET sync_status = 'error', sync_error = ?, updated_at = datetime('now') WHERE id = ?
...
app.post("/api/clients/:slug/ad-accounts/:accountId/ads/metrics", authMiddleware, requirePermission('ads.metrics'), async (c) => {
  ...
  try {
    ...
    const { ad_ids, days = 7, banco_first, batch_info } = requestBody;
    ...
    // STEP 6: ULTRA-FAST CACHE LOOKUP
    console.log(`[METRICS-HIGH-VOLUME-${requestId}] 🎯 Starting ultra-fast cache lookup for ${adCount} ads...`);
    
    const metricsCache = new MetricsCache(c.env.DB);
    const cacheResults = await metricsCache.getFromCache(ad_ids, days);
    ...
    if (shouldTryAPI && isPlatformSupported(adAccount.platform, 'metricsSupported')) {
      ...
      try {
        ...
        // Promise com timeout
        const apiPromise = platform.getMetrics(accessToken, adAccount.account_id, limitedMissingAds, days);
        const timeoutPromise = new Promise((_, reject) => {
      ...
      if (selectionId) {
        try {
          ...
          if (selection && selection.selection_type !== 'adjust') {
            ...
            if (currentPausedCount === 1 && selection.status === SELECTION_STATUS.PENDING) {
              // First ad paused - mark as in progress
              await markSelectionInProgress(c.env.DB, selectionId, user.id, user.name);
              console.log(`[AD-PAUSE-DEBUG] ✅ Selection marked as IN_PROGRESS (first ad paused)`);
            } else if (currentPausedCount === totalAdsInSelection) {
              // All ads paused - mark as completed
              const executionData: SelectionExecutionData = {
                selection_id: selectionId,
                executed_by_user_id: user.id,
                executed_by_user_name: user.name,
              ...
              console.log(`[AD-PAUSE-DEBUG] ✅ Selection marked as COMPLETED (all ads paused automatically)`);
            }
          }
